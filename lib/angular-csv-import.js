...
csvImport.directive('ngCsvImport', function() {
	return {
		...
		template: function(element, attrs) {
			...
			'<div>' +
			'<div>' +
				'<input id="button-file-upload" name="button-file-upload" class="btn cta gray" type="file" '+ (multiple ? 'multiple' : '') +' accept="{{accept}}" style="width:0px; height: 0px; visibility: hidden; padding:0;"/>' +
				'<label for="button-file-upload"><span class="btn btn-default btn-file"><span class="btn-label">{{uploadButtonLabel}}</span> <i class="fa fa-download" aria-hidden="true"></i></span></label>' +
			'</div>'+
